...
static const char* test_sensor_paths() {
	...
	while (tmp != NULL) {
		...
		if (tmp->path != NULL) {
			mu_assert("Sensor does not have valid temperature",
					tmp->temperature > 0);
		}
...
static const char* test_config_file() {
	...
	mu_assert("Could not read low_temp from config file",
			settings_get_int(settings, "general", "low_temp") != 0);
	mu_assert("Could not read high_temp from config file",
			settings_get_int(settings, "general", "high_temp") != 0);
	mu_assert("Could not read max_temp from config file",
			settings_get_int(settings, "general", "max_temp") != 0);
	mu_assert("Could not read polling_interval from config file",
			settings_get_int(settings, "general", "polling_interval") != 0);
...
static const char* test_settings() {
	...
	mu_assert("min_fan2_speed value is not 2000",
			fan->next->fan_min_speed == 2000);
