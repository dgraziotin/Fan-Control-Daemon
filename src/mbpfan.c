...
bool is_modern_sensors_path() {
	...
	if (atoi(str_kernel_version) < 3) {
		mbp_log(LOG_ERR,
				"mbpfan detected a pre-3.x.x linux kernel. Detected version: %s. Exiting.\n",
				kernel.release);
		exit(EXIT_FAILURE);
	}
	...
	for (counter = 0; counter < NUM_HWMONS; counter++) {
		...
		for (temp = 1; temp < NUM_TEMP_INPUTS; ++temp) {
			char *path =
					smprintf(
							"/sys/devices/platform/coretemp.0/hwmon/hwmon%d/temp%d_input",
							counter, temp);
...
void check_requirements(const char *program_path) {
	...
	if (uid != 0 || euid != 0) {
		mbp_log(LOG_ERR,
				"%s needs root privileges. Please run %s as root. Exiting.",
	...
	if (ENOENT == errno) {
		mbp_log(LOG_ERR,
				"%s needs coretemp support. Please either load it or build it into the kernel. Exiting.",
				program_path);
		exit(EXIT_FAILURE);
	}
	...
	if (ENOENT == errno) {
		mbp_log(LOG_ERR,
				"%s needs applesmc support. Please either load it or build it into the kernel. Exiting.",
				program_path);
		exit(EXIT_FAILURE);
	}
...
void mbpfan() {
	...
	while (fan != NULL) {
		...
		if (fan->fan_min_speed > fan->fan_max_speed) {
			mbp_log(LOG_ERR, "Invalid fan speeds: %d %d", fan->fan_min_speed,
					fan->fan_max_speed);
			exit(EXIT_FAILURE);
		}
		fan = fan->next;
	...
	if (low_temp > high_temp || high_temp > max_temp) {
		mbp_log(LOG_ERR, "Invalid temperatures: %d %d %d", low_temp, high_temp,
				max_temp);
		exit(EXIT_FAILURE);
	}
	...
	while (fan != NULL) {
		...
		fan->step_up = (float) (fan->fan_max_speed - fan->fan_min_speed)
				/ (float) ((max_temp - high_temp) * (max_temp - high_temp + 1)
						/ 2.0);

		fan->step_down = (float) (fan->fan_max_speed - fan->fan_min_speed)
				/ (float) ((max_temp - low_temp) * (max_temp - low_temp + 1)
						/ 2.0);
		fan = fan->next;
	}

	recalibrate: if (verbose) {
	...
	while (1) {
		...
		while (fan != NULL) {
			...
			if (temp_change > 0 && new_temp > high_temp
					&& new_temp < max_temp) {
				steps = (new_temp - high_temp) * (new_temp - high_temp + 1) / 2;
				fan_speed = max(fan_speed,
						ceil(fan->fan_min_speed + steps * fan->step_up));
			}

			if (temp_change < 0 && new_temp > low_temp && new_temp < max_temp) {
				steps = (max_temp - new_temp) * (max_temp - new_temp + 1) / 2;
				fan_speed = min(fan_speed,
						floor(fan->fan_max_speed - steps * fan->step_down));
			}

			if (verbose) {
				mbp_log(LOG_INFO,
						"Old Temp: %d New Temp: %d Fan: %s Speed: %d Max MHz: %d",
		...
		if (after_sleep - before_sleep > 2 * polling_interval) {
			mbp_log(LOG_INFO,
					"Clock skew detected - slept for %ld seconds but expected %d",
